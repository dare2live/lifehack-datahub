...
已经人工核对的 outcome 结论可以沉淀到 `config/outcome_collection_review_seeds.json`，再重放到重新生成的采集计划。种子只保存指标值、来源 URL、摘录、口径和复核说明，不提交 PDF 原文或 ignored staging 文件；`audit-outcome-collection-review-seeds` 会按 `config/outcome_metrics.json` 的 metric 注册检查指标合法性、完成状态、重复主键、metric_year 整数、日期格式、数值类型和值域上下限，避免 120%、负数、`2024.0` 年份或非 `YYYY-MM-DD` 日期这类错误进入后续计划：
