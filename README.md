...
上述 25 条复核结论已沉淀为 `config/career_source_review_seeds.json`，只记录职业、指标、年份、城市、状态和复核理由，不提交原始职位明细。薪酬调查和招聘快照类种子允许携带已核对的 `metric_value/source_title/source_url/evidence_quote/metric_scope`，用于从受控报告直接重放完整职业信号。`audit-career-source-review-seeds` 可检查种子合法性，`apply-career-source-review-seeds` 可把种子重放到重新生成的 `career_source_plan`；当前种子共 42 条，其中 25 条来自国考职位表、15 条来自宁波 2024 年薪酬调查、2 条来自广州 2025 年第四季度人力资源市场供求分析。
