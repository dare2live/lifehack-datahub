"""Build and merge local review batches for city context collection plans."""
...
from datahub.builders.city_context_collection_plan import PLAN_COLUMNS
from datahub.config import load_city_context_collection


TASK_KEY_COLUMNS = ("domain", "adcode", "metric_key", "metric_year")


def build_city_context_review_batch(
    *,
    plan_csv: Path,
    output_dir: Path,
    ...
    csv_path = output_dir / "city_context_review_batch.csv"
    manifest_path = output_dir / "city_context_review_batch.json"
    ...
    manifest = {
        ...
        "notes": "Local city context batch only. Merge edited rows back into the full city context collection plan before audit/package construction.",
    }
...
def merge_city_context_review_batch(
    *,
    plan_csv: Path,
    batch_csv: Path,
    output: Path,
    ...
    domain_counts = Counter(str(row.get("domain") or "") for row in plan_rows)
    ...
    return {
        ...
        "domain_counts": dict(sorted(domain_counts.items())),
        "status_counts": dict(sorted(status_counts.items())),
        "notes": "Merged city context batch into the full collection plan. Run audit-city-context-collection-plan next.",
    }
...
def _batch_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(batch, dict):
        raise ValueError("city_context_collection.review_batch is required")
    ...
    if missing:
        raise ValueError(f"city_context_collection.review_batch missing list config: {', '.join(missing)}")
    limit = batch.get("limit_per_domain")
    if not isinstance(limit, int) or limit < 1:
        raise ValueError("city_context_collection.review_batch.limit_per_domain must be a positive integer")
    return {
