"""Build a reviewed delete-migration plan for score-history core-only rows."""
...
from datahub.builders.score_history_reconciliation_package import _has_core_side, _json_value
...
def build_score_history_delete_plan_from_reconciliation_plan(
    *,
    plan_csv: Path,
    output_dir: Path,
) -> dict[str, Any]:
    ...
    if not readiness["ready"]["review_complete"] or progress.get("blocked_rows") or progress.get("blocking_decision_rows") or readiness.get("errors"):
        raise ValueError(
    ...
    csv_path = output_dir / "score_history_delete_plan.csv"
    manifest_path = output_dir / "score_history_delete_plan.json"
    _write_csv(csv_path, rows)
    issue_counts = Counter(str(row.get("review_decision") or "") for row in rows)
    manifest = {
...
def _delete_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "score_year": core_key.get("score_year") or row.get("score_year") or "",
        "min_score": row.get("core_min_score") or "",
        "min_rank": row.get("core_min_rank") or "",
