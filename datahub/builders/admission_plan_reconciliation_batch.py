"""Build small review batches from admission-plan reconciliation plans."""
...
from datahub.builders.admission_plan_reconciliation_audit import _review_config
from datahub.builders.admission_plan_reconciliation_plan import PLAN_COLUMNS
from datahub.config import get_table_schema


def build_admission_plan_reconciliation_review_batch(
    *,
    plan_csv: Path,
    output_dir: Path,
    ...
    for issue_type in _issue_type_order(review_config, selected_issue_types):
        issue_rows = sorted(
            ...
            key=lambda row: (
                ...
                str(row.get("core_major_code") or ""),
    ...
    csv_path = output_dir / "admission_plan_reconciliation_review_batch.csv"
    manifest_path = output_dir / "admission_plan_reconciliation_review_batch.json"
    ...
    manifest = {
        ...
        "notes": "Local review batch only. Merge reviewed rows back into the reconciliation plan before migration/package decisions.",
    }
...
def merge_admission_plan_reconciliation_review_batch(
    *,
    plan_csv: Path,
    batch_csv: Path,
    output: Path,
    ...
    return {
        ...
        "notes": "Merged review batch into full reconciliation plan. Run audit-admission-plan-reconciliation-plan next.",
    }
