...
core-only 与 package-only 存在唯一精确专业名配对时，使用 `apply-score-history-pair-name-reference-decisions`。该命令把 package 行改为 `map_package_to_core_major_code`，把 core-only 行标记为 `covered_by_mapped_package_row`；包构建时前者输出一条使用 core `major_code` 的官方分数/位次行，后者被跳过且不会进入 delete plan。
