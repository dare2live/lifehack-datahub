...
多候选专业代码漂移可继续用官方投档 CSV 的 `major_full` 和 core 招生计划候选专业名做只读参考。`apply-score-history-major-name-reference-decisions` 只接受唯一精确专业名匹配，并把 `core_candidates_json/core_key_json/core_major_code` 收敛到被命中的候选；无命中、多个命中或仅包含匹配都保持 pending，避免把跨年份名称近似当作自动结论。

core-only 与 package-only 存在唯一精确专业名配对时，使用 `apply-score-history-pair-name-reference-decisions`。该命令把 package 行改为 `map_package_to_core_major_code`，把 core-only 行标记为 `covered_by_mapped_package_row`；包构建时前者输出一条使用 core `major_code` 的官方分数/位次行，后者被跳过且不会进入 delete plan。若同名 package 侧已经是 `value_drift/reviewed/use_package_row`，命令会改用 `map_package_to_core_major_code_delete_original_core`：数据包输出目标 core 专业代码，delete plan 单独记录原 core 主键，避免旧代码残留。
...
2022 官方派生包 reconciliation 已用同一规则继续降噪：16,963 条任务中 3,638 条零占位、5,346 条 package-only、3,712 条 value drift 和 2,047 条单候选专业代码漂移被自动标为 reviewed，剩余 2,220 条仍为 todo（1,773 条 core-only、447 条多候选专业代码漂移）。随后用官方投档专业名与 core 2026 招生计划候选专业名做唯一精确匹配，又将 170 条多候选专业代码漂移收敛为 reviewed；再用精确专业名配对处理 908 对成对漂移，其中 483 对来自 package-only，425 对来自已复核 value drift 并生成旧 core 主键 delete-plan 语义。当前 reviewed 为 15,821/16,963，剩余 1,142 条 todo（865 条 core-only、277 条多候选专业代码漂移），并已基于新 plan 生成 120 行小批复核包。后续人工或证据核验完成前仍不能生成可导入包或 delete plan。
...
职业复核种子分两类：国考职位表种子只保存复核结论，证据仍来自重新生成的职位匹配计划；薪酬调查、招聘快照等受控报告类种子必须携带 `config/career_data_sources.json.audit.required_seed_copy_fields_by_source` 指定的指标值、口径、来源、摘录和日期字段，用于重放完整信号。种子审计按指标注册和来源策略校验年份整数、日期格式、`source_date <= availability_date <= reviewed_at`、HTTP(S) 来源 URL 和 `metric_value` 上下限，避免坏证据进入 `career_source_plan` 重放链路。国考职位表真实 run：以本地 core 73 条职业目录生成 44 条候选复核行，当前 25 条已核职业信号生成 `2026_career_signal_civil_service_verified_v2` 和 `2026_career_score_civil_service_verified_v2` 标准包，manifest 校验、core importer dry-run 和本地实导均通过；19 条候选保持 `in_progress`。薪酬调查真实 run：以本地 core 73 条职业目录生成宁波 2024 年 `career_salary_survey` 采集计划 219 行，重放 15 条宁波薪酬调查种子，覆盖 5 个职业的 `salary_p25/salary_median/salary_p75`；审计无错误，生成 `2024_ningbo_salary_career_signal_v1` 15 行和 `2024_ningbo_salary_career_score_v1` 5 行，manifest 校验、core importer dry-run 和本地实导均通过。招聘快照首批真实 run：以本地 core 73 条职业目录生成广州 2025 年 `career_recruitment_snapshot` 采集计划 365 行，重放广州市人社局公开供求分析中的 2 条 `shortage_rank` 种子，生成 `2025_guangzhou_shortage_career_signal_v1` 2 行和 `2025_guangzhou_shortage_career_score_v1` 2 行；该评分包保留 `below_minimum_signal_count`，用于提示当前只有单一紧缺排行证据。

招聘平台来源按 `config/career_data_sources.json.platform_source_policy` 分层治理。公开研究报告可作为职业需求、薪资、紧缺度和工作强度候选；授权 API 或金融终端导出必须保留授权来源、指标定义、导出时间和文件 hash；政府市场报告优先级高于商业平台报告；公开页面快照默认只作研究候选；社区爬虫和未授权接口不能进入标准包。所有招聘平台证据都禁止直接发布到 `fa_fact_school_outcome` 或 `fa_fact_major_outcome`，只能在复核后进入 `fa_fact_career_signal`、`fa_mart_career_score` 或专业城市就业适配等机会证据层。
